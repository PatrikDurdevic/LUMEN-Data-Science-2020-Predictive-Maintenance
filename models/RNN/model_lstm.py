...
def create_paths(main_1_path, main_2_path, model_path, model_machine_path, figures_path):
	if not os.path.exists(figures_path):
		try:
			...
			print(f"Successfully created the directory {model_path}")
		try:
			os.makedirs(model_machine_path)
		except OSError:
			print(f"{model_machine_path} already exists")
		else:
			print(f"Successfully created the directory {model_machine_path}")
		try:
			os.makedirs(figures_path)
		except OSError:
			print(f"{figures_path} already exists")
		else:
			print(f"Successfully created the directory {figures_path}")
	else:
		print("Directories already exist!")

def train_machine(df_machine, machine, single_step_model, target_index):
	...
	main_1_path = f"models"
	main_2_path = f"{main_1_path}/per_machine"
	model_path = f"{main_2_path}/{machine}"
	model_machine_path = f"{model_path}/{target_index_name}"
	figures_path = f"{model_machine_path}/figures"
	create_paths(main_1_path, main_2_path, model_path, model_machine_path, figures_path)
	...
	data_mean = dataset[:TRAIN_SPLIT_INT].mean(axis=0)
	data_std = dataset[:TRAIN_SPLIT_INT].std(axis=0)
	...
	log_dir = f"logs/fit/{machine}_{target_index_name}_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
	...
	single_step_model.save(f"{model_machine_path}/{machine}.h5")
...
for m in range(2, 7):
	for i in range(0, 12, 1):
		...
		train_machine(df_machine, f"FL0{m}", single_step_model, i)


for i in range(0, 12, 1):
	single_step_model = get_model()
	train_machine(df_machine, f"FL01_before", single_step_model, i)
	train_machine(df_machine, f"FL01_after", single_step_model, i)

for i in range(0, 12, 1):
	single_step_model = get_model()
	train_machine(df_machine, f"FL07_before", single_step_model, i)
	train_machine(df_machine, f"FL07_after", single_step_model, i)
